  ...
  (let ((-style
    ...
    (insert
     (cond
      ...
      ((= -style 1)
       (format-time-string "%Y-%m-%d") ; "2016-10-10 Monday"
       )
      ((= -style 2)
       (concat
...
(defun update-layout-map-with-ru-keys (to-list)
  (let (ru-elem)
    (setq ru-elem (delq nil (mapcar
	    (lambda (elem)
	      (let ((ru-key (cdr (assoc (car elem) ru-layout-alist)))
		    (ru-value (cdr elem)))
		(if (not (eq ru-key nil)) (cons ru-key ru-value))
     
		)) to-list)))
    ))

(defun generate-ru-shortcuts ()
  (interactive)
  (mapcar (lambda (elem)
	    (let* ((key (car elem))
		  (value (cdr elem)))
	      (define-key xah-fly-key-map (kbd (concat "C-" value)) `(lambda () (interactive) (execute-kbd-macro (kbd (concat "C-" ',key)))))
	      )	    
	    ) ru-layout-alist)
  )
...
Version 2017-01-21"
  (interactive)
  ;;(setq *key-cmd-alist (append *key-cmd-alist (update-layout-map-with-ru-keys *key-cmd-alist)))
  (let (new-key-alist)
    (setq new-key-alist (append (update-layout-map-with-ru-keys *key-cmd-alist) *key-cmd-alist))
...
(xah-fly--define-keys
 ...
 '(
   ("SPC" . whitespace-mode)
   ("r" . auto-revert-mode) ; toggle-read-only
   ("v" . visual-line-mode)
   ("s" . shell)
...
Version 2017-01-21"
  ...
  (xah-fly--define-keys
   ...
   '(
     ...
     ("N" . (lambda () (interactive) (execute-kbd-macro (kbd "M-,"))))
     ...
     ("i" . previous-line)
     ("h" . (lambda () (interactive) (xah-beginning-of-line-or-block)))
     ("d" . xah-delete-backward-char-or-bracket-text)
     ("y" . isearch-forward)
     ("u" . backward-word)
     ("j" . backward-char)
     ("J" . switch-window-mvborder-left)
     ("g" . undo)
     ("G" . undo-tree-redo)
     ("c" . xah-copy-line-or-region)
     ("I" . switch-window-mvborder-up)
     ("v" . xah-paste-or-paste-previous)
     ...
     (";" . (lambda () (interactive) (xah-end-of-line-or-block)))
     ("k" . next-line)
     ("K" . switch-window-mvborder-down)
